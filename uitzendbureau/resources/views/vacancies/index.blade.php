...
@section('content')
    <div class="px-5">
        <div class="d-flex align-items-center gap-2 justify-content-between flex-wrap">
            <h1 class="mb-0">Vacature Lijst</h1>
            <a href="{{ route('vacancies.create') }}" class="btn btn-primary mt-ms-0">Nieuwe vacature</a>
        </div>
        <hr>

        @if ($vacancies->isEmpty())
            <p>Er zijn nog geen vacature toegevoegd.</p>
        @else
            <table class="table table-hover table-sm">
                <thead class="table-secondary">
                <tr>
                    <form method="GET" action="{{ route('vacancies.index') }}" id="sort_form">
                        ...
                        <th scope="col"><button type="button" class="text_button" onclick="toggleSort('jobTitle')"><b id="jobTitle">Positie</b></button></th>
                        <th scope="col"><button type="button" class="text_button" onclick="toggleSort('jobDescription')"><b id="jobDescription">Beschrijving</b></button></th>
                        <th scope="col"><button type="button" class="text_button" onclick="toggleSort('employer_id')"><b id="employer_id">Werkgever</b></button></th>
                        <th scope="col"><button type="button" class="text_button" onclick="toggleSort('salary')"><b id="salary">Salaris</b></button></th>
                ...
                @foreach ($vacancies as $vacancy)
                    <tr>
                        <th scope="row">{{ $vacancy->jobTitle }}</th>
                        <td>{{ $vacancy->jobDescription }}</td>
                        <td>{{ $vacancy->employer->companyName }}</td>
